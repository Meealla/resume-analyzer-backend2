...
public class TestForHobbyController {
    ...
    private final String uriTemp = "/hobbies";
    private final UUID generatedId = UUID.randomUUID();
    private final Hobby testJson = new Hobby(generatedId, "testHobby");
    ...
    /**
     * Экземпляр контроллера {@link HobbyTestController}, в который внедряется Mock сервис.
     */
    @InjectMocks
    ...
    public void testCreateHobby() throws Exception {
        ...
        when(hobbyService.createHobby(any(Hobby.class))).thenReturn(testJson);
